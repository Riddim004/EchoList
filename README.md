...
- 🧠 **AI 语义解析** — 智谱 GLM-4.7-Flash（免费，思考模式）提取时间、事件、分类；一句话多个任务自动拆分
...
TaskAgent (编排器)
   ...
   ▼
GLM-4.7-Flash ──── tool_calls ──► ToolCallingHarness (ToolRegistry)
   ▲                                  │ 参数校验 / 并行执行 / 失败回传自纠错
   │◄──────── role=tool 结果 ─────────┤
   │                                  ▼
   │                    ┌─────────────────────────────┐
...
| 模型 | 智谱 GLM-4.7-Flash（免费，混合思考模型，已开启思考模式），Retrofit + kotlinx.serialization |
